...
Now we are rolling a dice a hundred times.

Our *mathematical distribution* tells us that, with a fair dice, the probability of getting 1, 2, 3, 4, 5, and 6 are equal: there is a 1/6 chance of each.
...
ggplot(mydata, aes(x = dice_1)) +
  geom_histogram(aes(y=..count../sum(..count..)), binwidth = 0.5, 
                 fill = "dark gray", color = "white") +
  labs(title = "Mathematical Probability Distribution",
       subtitle = "Rolling One Dice A Hundred Times",
       x = "Outcomes",
       y = "Probability") +
...
```
We get an *empirical distribution* by running a simulation and flipping the coin a hundred times. The printed tibble shows us that we rolled 21 6's, but only 12 3's. The p(1) is not equal to p(2) and almost every outcome occurred a different number of times.
...
```
The *posterior distribution* for rolling one dice a hundred times will vary based on your expectations. If you take the dice from your Monopoly set, you have reason to believe that the mathematical distribution is true. However, if you walk into a crooked casino and a host offers you money when you roll a number above 2, you might believe that 1's and 2's are more common than anything else.
