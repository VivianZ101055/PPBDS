...
count_1 <- mydata %>%
  group_by(die.1) %>%
  summarize(number_die1 = n()) %>%
  select(number_die1)

count_2 <- mydata %>%
  group_by(die.2) %>%
  summarize(number_die2 = n()) %>%
  select(number_die2)
...
mydata <- merge(mydata, otherdata)
mydata <- merge(mydata, count_1)
mydata <- merge(mydata, count_2)

# mydata$die.1
# 
# hist3D(x = mydata$number_die1, y = mydata$number_die2, z = z_factor)
# 
# 
# joint_distribution <- ggplot(mydata, aes(x = die.1, y = die.2)) +
...
# 
# bayes_scatter <- ggplot(mydata, aes(x = die.1, y = die.2)) +
#   geom_point() +
#   geom_jitter()
# 
# mydata
